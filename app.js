...
function showSection(id){
  if(isKidsProfile() && !["home","kids-section","kids-series","my-list","live-tv","watch-party","account"].includes(id)){showToast("This area is locked for the Kids profile.","error");return;}
...
function renderKidsSeries(){
  ...
  box.innerHTML=titles.length?titles.map(t=>`<article class="library-card kids-series-card" data-title="${safe(t)}"><div class="art"><img src="${SHOWS[t].image}" alt="${safe(t)}"></div><div class="library-card-copy"><h3>${safe(t)}</h3><p>Kids • ${SHOWS[t].episodes?.length||0} episodes${SHOWS[t].live?" • Live TV":""}</p><button type="button" class="watch-btn kids-series-view" data-kids-series-title="${safe(t)}">View Show</button></div></article>`).join(""):`<div class="kids-empty"><h3>No kids shows yet</h3><p>More kid-friendly shows will appear here when you add them.</p></div>`;
...
async function init(){
  ...
  $("#accountForm").onsubmit=async e=>{
    ...
    user=r.data.user;signup=false;await loadProfile();loadProfilesFromUser();await loadMembership();accountUI();setAccountMessage("Signed in successfully.","success");await renderList();await renderHistory();await renderContinue();await refreshProfileDrawerCounts();
  };
