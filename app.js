...
let profileList = [];
const KIDS_SHOWS = new Set(["The Proud Family","Static Shock"]); const KIDS_PLUS_SHOWS = new Set(["The Proud Family Movie"]);
...
const SHOWS = {
  ...
  "Afro Samurai: Resurrection": {
    type:"movie", premium:true, live:false, kid:false,
    description:"Afro Samurai: Resurrection (2009).",
    image:"afro-samurai-thumbnail.jpg",
    movie:true,
    video:`${VIDEO_BASE}/Afro%20Samurai/Afro.Samurai.Resurrection.2009.1080p.BluRay.H264.AAC-RARBG.mp4`
  },
  "The Proud Family Movie": {
    type:"movie", premium:true, live:false, kid:true,
    description:"The Proud Family Movie (2005).",
    image:"proud-family-thumbnail.jpg",
    movie:true,
    video:`${VIDEO_BASE}/The%20Proud%20Family/The.Proud.Family.Movie.2005.1080p.AMZN.WEB-DL.DDP5.1.H.264-NTb.mkv.mp4`
  },
  "Static Shock": {
...
function renderExistingMovieCategories(){
  const adultMovieSelectors=["#movies .poster-row","#moviesContent","#movieContent"];
  const adultMovieBox=adultMovieSelectors.map(sel=>$(sel)).find(Boolean);
  if(adultMovieBox){
    const movies=Object.keys(SHOWS).filter(t=>SHOWS[t].type==="movie"&&!SHOWS[t].kid);
    adultMovieBox.innerHTML=movies.map(t=>{const m=SHOWS[t];return `<article class="wide-card" data-title="${safe(t)}" data-type="movie"><div class="art addon-card-art"><img src="${m.image}" alt="${safe(t)}"></div><h3>${safe(t)}</h3><p>${m.premium?"Premium":"UrbanAnimeTv"} • Movie</p></article>`;}).join("");
    $$("#movies .wide-card[data-type=\"movie\"], #moviesContent .wide-card[data-type=\"movie\"], #movieContent .wide-card[data-type=\"movie\"]").forEach(card=>card.onclick=e=>{e.stopPropagation();openDetails(card.dataset.title);});
  }

  const kidsMovieSelectors=["#kidsMoviesContent","#kidsMovies","#kids-movies"];
  const kidsMovieBox=kidsMovieSelectors.map(sel=>$(sel)).find(Boolean);
  if(kidsMovieBox){
    const movies=Object.keys(SHOWS).filter(t=>SHOWS[t].type==="movie"&&isKidsAllowed(t));
    kidsMovieBox.innerHTML=movies.map(t=>{const m=SHOWS[t];return `<article class="wide-card kids-card" data-title="${safe(t)}" data-type="movie"><div class="art"><img src="${m.image}" alt="${safe(t)}"></div><h3>${safe(t)}</h3><p>Kids • Movie</p></article>`;}).join("");
    $$("#kidsMoviesContent .wide-card, #kidsMovies .wide-card, #kids-movies .wide-card").forEach(card=>card.onclick=e=>{e.stopPropagation();openDetails(card.dataset.title);});
  }
}
...
function ensureKidsMoviesSection(){
  const kidsSection=$("#kids-section");
  if(!kidsSection)return null;
  let section=$("#kidsMoviesSection");
  if(!section){
    section=document.createElement("section");
    section.id="kidsMoviesSection";
    section.className="home-section kids-movies-section";
    section.innerHTML=`<div class="section-head"><div><span class="section-kicker">UATV KIDS</span><h2>Movies</h2></div></div><div id="kidsMoviesContent" class="poster-row"></div>`;
    const series=kidsSection.querySelector("#kidsSeriesContent")?.closest("section");
    const content=kidsSection.querySelector("#kidsContent");
    if(series)series.parentElement.insertBefore(section,series);
    else if(content)content.parentElement.insertAdjacentElement("afterend",section);
    else kidsSection.appendChild(section);
  }
  return section;
}
...
function renderKidsSection(){
  ...
  const kidsMoviesSection=ensureKidsMoviesSection();
  const kidsMoviesBox=kidsMoviesSection?.querySelector("#kidsMoviesContent");
  const kidMovies=Object.keys(SHOWS).filter(t=>SHOWS[t]?.type==="movie"&&isKidsAllowed(t));
  if(kidsMoviesBox)kidsMoviesBox.innerHTML=kidMovies.length?kidMovies.map(t=>`<article class="wide-card kids-card" data-title="${safe(t)}" data-type="movie"><div class="art"><img src="${SHOWS[t].image}" alt="${safe(t)}"></div><h3>${safe(t)}</h3><p>Kids • Movie</p></article>`).join(""):`<div class="kids-empty"><h3>No kids movies yet</h3><p>Kids movies you add will appear here.</p></div>`;
  $$("#kidsMoviesContent .wide-card").forEach(c=>c.onclick=()=>openDetails(c.dataset.title));
