...
function showHome(){
  if(isKidsProfile()){showSection("kids-section");return;}
...
function showSection(id){
  if(isKidsProfile() && !["kids-section","my-list","live-tv","watch-party","account"].includes(id)){showToast("This area is locked for the Kids profile.","error");return;}
...
function getNextPlayableEpisode(title,seasonNumber,episodeNumber){
  ...
  if(index>=0 && episodes[index+1])return {season:Number(seasonNumber),episode:episodes[index+1]};
  const show=SHOWS[title];
...
function getPreviousPlayableEpisode(title,seasonNumber,episodeNumber){
  const episodes=getPlayableEpisodes(title,seasonNumber);
  const index=episodes.findIndex(ep=>ep.number===Number(episodeNumber));
  if(index>0)return {season:Number(seasonNumber),episode:episodes[index-1]};
  const show=SHOWS[title];
...
function ensureNextEpisodeButton(){
  ...
  if(!meta)return null;
  let btn=document.getElementById('playerNextEpisode');
  if(!btn){
    btn=document.createElement('button');
    btn.id='playerNextEpisode';
    btn.type='button';
    ...
    btn.addEventListener('click',()=>{
      const next=btn._nextEpisode;
      if(!next)return;
      play(btn.dataset.title,next.episode,next.season);
    });
  }
  return btn;
}

function updateNextEpisodeButton(title,season,episode){
  const btn=ensureNextEpisodeButton();
  if(!btn)return;
  const next=getNextPlayableEpisode(title,season,episode);
  btn._nextEpisode=next;
  btn.dataset.title=title;
  if(next){
    btn.disabled=false;
    btn.classList.remove('hidden');
    btn.textContent=`Next Episode → S${next.season} E${next.episode}`;
  }else{
    btn.disabled=true;
...
function play(title="HoodGods",episode=1,season=1){
  ...
  const nextButton=updateNextEpisodeButton(title,season,episode);
  if(video && ep?.video){
    saveHistory(title,0,0,false);
    video.onended=()=>{
      if(nextButton){
        nextButton.classList.remove('hidden');
        nextButton.disabled=!nextButton._nextEpisode;
      }
...
function applyProfileMode(){
  ...
  $$(".nav-link").forEach(a=>{
    ...
    const kidsAllowedNav=["home","kids-section","my-list","live-tv","watch-party"];
    a.classList.toggle("profile-hidden",(kids && !kidsAllowedNav.includes(nav)) || (!kids && ["live-tv","watch-party"].includes(nav)));
  });
...
function renderKidsSection(){
  ...
  box.innerHTML=kidTitles.length?kidTitles.map(t=>`<article class="wide-card kids-card" data-title="${safe(t)}"><div class="art"><img src="${SHOWS[t].image}" alt="${safe(t)}"></div><h3>${safe(t)}</h3><p>Kids • ${SHOWS[t].live?"Live TV":"Series"}</p></article>`).join(""):`<div class="kids-empty"><div class="kids-empty-mark">UATV KIDS</div><h3>No kids shows yet</h3><p>More kid-friendly shows will appear here when you add them.</p></div>`;
...
async function init(){
  ...
  $("#profileManagerAdd").onclick=()=>{if(profileList.length>=3){showToast("You can have up to 3 profiles.","error");return;} profileList.push({id:`profile-${Date.now()}`,name:`Profile ${profileList.length+1}`,type:"adult",pinHash:""});renderProfileManager();};
  ...
  $("#profileManagerSwitch").onclick=()=>{closeProfileManager();openProfileManager();};
...
async function auth(){
  ...
  supabase.auth.onAuthStateChange(async(_e,s)=>{
    user=s?.user||null;
    if(user)await loadProfile();else{profile={display_name:"",avatar_url:""}; profileList=[];activeProfile={id:"default",name:"Main",type:"adult",pinHash:""};membership={plan:"free",status:"active"};setTopAvatar("");}
    await loadMembership();
