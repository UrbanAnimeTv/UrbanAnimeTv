...
const SHOWS = {
  ...
  "The PJs": {
    ...
    live:true,
    seasons:[{number:1,episodes:[]},{number:2,episodes:[]},{number:3,episodes:[]}]
  },
  "The Boondocks": {
    type:"series",
...
function renderLiveTV(){
  ...
  box.innerHTML=liveTitles.map(title=>{
    ...
    return `<article class="live-card" data-live-card="${safe(title)}">
  ...
  $$(".live-watch").forEach(btn=>btn.onclick=e=>{
    e.stopPropagation();
    showToast(`${btn.dataset.liveTitle} is set up as a Live TV channel.`);
  });
  $$(".live-library").forEach(btn=>btn.onclick=e=>{
    e.stopPropagation();
    toggleList(btn.dataset.listTitle);
  });
  $$("[data-live-card]").forEach(card=>card.onclick=e=>{
    if(e.target.closest("button"))return;
    showToast(`${card.dataset.liveCard} is available in Live TV.`);
  });
