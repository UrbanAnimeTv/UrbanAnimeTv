...
let profileList = [];
const KIDS_SHOWS = new Set(["The Proud Family","Static Shock","The Proud Family Movie"]); const KIDS_PLUS_SHOWS = new Set();
...
const SHOWS = {
  ...
  "The PJs": {
    ...
    seasons:[
      {number:1,episodes:Array.from({length:15},(_,i)=>({number:i+1,title:`Episode ${i+1}`,description:"Premium episode",video:i<14?videoUrl(`The PJs/The.PJs.s01e${String(i+1).padStart(2,"0")}.480p.mp4`):null}))},
  ...
  "Black Dynamite": {
    ...
    {number:2,episodes:Array.from({length:6},(_,i)=>({number:i+1,title:`Episode ${i+1}`,description:`Black Dynamite Season 2 Episode ${i+1}.`,video:videoUrl(`Black Dynamite/Black.Dynamite.S02E0${i+1}.1080p.HMAX.WEB-DL.DD5.1.H.264-PlayWEB.mkv.mp4`)}))}
  ]}
  },
  "The Cleveland Show": {
    type:"series", premium:true, live:true, channel:true,
  ...
  "Just Jordan": {
    ...
    {number:2,episodes:[
      ...
      ...Array.from({length:9},(_,i)=>({number:i+8,title:`Episode ${i+8}`,description:`Just Jordan Season 2 Episode ${i+8}.`,video:videoUrl(`Just Jordan/Just.Jordan.S02E${String(i+8).padStart(2,"0")}.360p.mp4`)}))
    ]}]
  },
  ...
  ,"The Proud Family Movie": {
    type:"movie", premium:true, live:false, kid:true,
    description:"The Proud Family Movie — Penny and the Proud family take on a big-screen adventure full of family, friendship and fun.",
    image:"proud-family-thumbnail.jpg",
    video:`${videoUrl("The Proud Family/The.Proud.Family.Movie.2005.1080p.AMZN.WEB-DL.DDP5.1.H.264-NTb.mkv.mp4")}`
  },
  "Afro Samurai: Resurrection": {
    type:"movie", premium:true, live:false,
    description:"Afro Samurai: Resurrection — Afro returns for another cinematic journey of revenge, honor and survival.",
    image:"afro-samurai-thumbnail.jpg",
    video:`${videoUrl("Afro Samurai/Afro.Samurai.Resurrection.2009.1080p.BluRay.H264.AAC-RARBG.mp4")}`
  }
};
...
function openDetails(title){
  ...
  if(select)select.dataset.title=title;
  const isMovie=show.type==="movie";
  if(select){
    select.innerHTML=isMovie?`<option value="1">Movie</option>`:(show.seasons?.length
      ? show.seasons.map(s=>`<option value="${s.number}">Season ${s.number}</option>`).join("")
      : `<option value="1">Season 1</option>`);
    select.disabled=isMovie;
    select.closest(".season-select-wrap, .season-select, label")?.classList.toggle("hidden",isMovie);
  }
  if(isMovie){
    const list=$("#episodeList");
    if(list)list.innerHTML=`<div class="episode-empty-premium"><div class="episode-number">MOVIE</div><h3>${safe(title)}</h3><p>Full movie — press Watch Now to start watching.</p></div>`;
  }else{
...
function renderEpisodes(title,seasonNumber){
  ...
  list.innerHTML=[...season.episodes].sort((a,b)=>Number(a.number)-Number(b.number)).map(ep=>{
...
function play(title="HoodGods",episode=1,season=1){
  ...
  const isMovie=show.type==="movie";
  const ep=isMovie?{number:1,title:title,description:show.description,video:show.video}:episodeFor(title,episode,season);
  ...
  if(!isMovie){
    const controls=document.getElementById("playerEpisodeControls");
...
function applyProfileMode(){
  ...
  $("#movies")?.classList.toggle("profile-hidden",kids);
...
function renderAdultMovies(){
  let section=$("#movies");
  if(!section){
    section=document.createElement("section"); section.id="movies"; section.className="page-section";
    section.innerHTML=`<div class="section-head"><div><span class="eyebrow">MOVIES</span><h2>Movies</h2><p>Movies available in the adult catalog.</p></div></div><div class="poster-row" id="adultMovieRow"></div>`;
    const anchor=$("#series"); anchor?.parentNode?.insertBefore(section,anchor.nextSibling);
  }
  const row=$("#adultMovieRow"); if(!row)return;
  row.innerHTML=Object.keys(SHOWS).filter(t=>SHOWS[t]?.type==="movie").map(title=>{const show=SHOWS[title];return `<article class="wide-card" data-title="${safe(title)}" data-type="movie"><div class="art addon-card-art"><img src="${show.image}" alt="${safe(title)}"></div><h3>${safe(title)}</h3><p>Premium • Movie</p></article>`;}).join("");
  $$("#adultMovieRow .wide-card").forEach(card=>card.onclick=()=>openDetails(card.dataset.title));
}
function renderKidsMovies(){
  if(!isKidsProfile())return;
  let section=$("#kids-movies");
  const kidsSection=$("#kids-section"); if(!kidsSection)return;
  if(!section){
    section=document.createElement("section"); section.id="kids-movies"; section.className="kids-movie-section";
    section.innerHTML=`<div class="section-head"><div><span class="eyebrow">UATV KIDS MOVIES</span><h2>Movies</h2><p>Kids movies are kept separate from TV shows.</p></div></div><div class="poster-row" id="kidsMovieRow"></div>`;
    kidsSection.appendChild(section);
  }
  const row=$("#kidsMovieRow"); if(!row)return;
  row.innerHTML=Object.keys(SHOWS).filter(t=>SHOWS[t]?.type==="movie"&&isKidsAllowed(t)).map(title=>{const show=SHOWS[title];return `<article class="wide-card kids-card" data-title="${safe(title)}"><div class="art"><img src="${show.image}" alt="${safe(title)}"></div><h3>${safe(title)}</h3><p>Kids • Movie</p></article>`;}).join("");
  $$("#kidsMovieRow .kids-card").forEach(c=>c.onclick=()=>openDetails(c.dataset.title));
}
...
function renderKidsSection(){
  const kidTitles=Object.keys(SHOWS).filter(t=>isKidsAllowed(t) && SHOWS[t]?.type!=="movie");
...
function renderKidsSeries(){
  ...
  const titles=Object.keys(SHOWS).filter(t=>isKidsAllowed(t) && SHOWS[t]?.type!=="movie");
...
async function init(){
  ...
  $("#seasonSelect").onchange=e=>{e.stopPropagation();const title=e.target.dataset.title||activeTitle;activeTitle=title;renderEpisodes(title,Number(e.target.value));};
