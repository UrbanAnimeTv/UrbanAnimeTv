...
function setActiveNav(id){$$(".nav-link").forEach(a=>a.classList.toggle("active",a.dataset.nav===id));}
...
async function renderList(){
  ...
  box.innerHTML=data.map(x=>`
    <article class="library-card">
      ...
      <div class="library-card-actions"><button class="watch-btn" data-library-watch="${safe(x.title)}">▶ Watch</button><button class="list-btn" data-remove-list="${safe(x.title)}">Remove</button></div></div>
    </article>`).join("");
...
async function uploadAvatar(file){
  ...
  if(uploadError){setProfileMessage(`Upload error: ${uploadError.message}`,"error");return;}
  const {data}=supabase.storage.from("avatars").getPublicUrl(path);
  const url=data.publicUrl;
  const {error}=await supabase.from("profiles").upsert({user_id:user.id,display_name:profile.display_name||"",avatar_url:url},{onConflict:"user_id"});
  if(error){setProfileMessage(error.message,"error");return;}
  profile.avatar_url=url+"?v="+Date.now();
  $("#profilePreview").src=profile.avatar_url;$("#profileAvatar").src=profile.avatar_url;setTopAvatar(profile.avatar_url);
...
async function auth(){
  ...
  supabase.auth.onAuthStateChange(async(_e,s)=>{
    ...
    accountUI();if(user){await renderList();await renderHistory();}else{updateListButtons("HoodGods",false);updateListButtons("City of Ash",false);}
