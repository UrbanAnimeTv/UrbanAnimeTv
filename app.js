  ...
  "Aqua Teen Hunger Force": {
    ...
    seasons:[{
      ...
      episodes:[{
        number:1,
        title:"Rabbot",
        description:"Aqua Teen Hunger Force Season 1 Episode 1.",
        video:"https://uatv-video.urbananimetv10.workers.dev/Aqua%20Teen%20Hunger%20Force/Aqua.Teen.Hunger.Force.S01E01.Rabbot.720p.mkv.mp4"
      },{
        number:2,
        title:"Escape from Leprechaunpolis",
        description:"Aqua Teen Hunger Force Season 1 Episode 2.",
        video:"https://uatv-video.urbananimetv10.workers.dev/Aqua%20Teen%20Hunger%20Force/Aqua.Teen.Hunger.Force.S01E02.Escape.from.Leprechaunpolis.720p.mkv.mp4"
      }]
    }]
  }
};
...
function play(title="HoodGods",episode=1,season=1){
  ...
  if(video){
    ...
    video.onerror=()=>{
      if(playerNote)playerNote.textContent="This video could not be played. Check the video source or hosting response.";
    };
    video.onloadedmetadata=()=>{
      if(playerNote)playerNote.textContent="Premium playback";
    };
    if(ep?.video){
      if(playerNote)playerNote.textContent="Loading video…";
      video.src=ep.video;
      video.load();
    }else{
...
document.addEventListener("click",e=>{
  ...
  if(liveWatch){e.preventDefault();if(requirePremium())openWatchParty(liveWatch.dataset.liveTitle);return;}

  const liveParty=target.closest("[data-party-title]");
  if(liveParty){e.preventDefault();if(requirePremium())openWatchParty(liveParty.dataset.partyTitle);return;}
});
